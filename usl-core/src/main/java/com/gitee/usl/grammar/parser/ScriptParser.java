...
import com.gitee.usl.grammar.lexer.token.PatternToken;
import com.gitee.usl.grammar.lexer.token.StringToken;
...
public class ScriptParser implements Parser {
    ...
    private boolean expectChar(final char ch) {
        ...
        return this.current.getType() == Token.TokenType.Char && ((CharToken) this.current).getCh() == ch;
    }
            ...
            if (expectChar('=')) {
                ...
                if (expectChar('=')) {
                    ...
                    if (prevToken.getType() == Token.TokenType.Variable) {
                        isVar = true;
                    } else if (prevToken.getType() == Token.TokenType.Char
    ...
    private void checkVarIsInit(final Token<?> prevToken) {
        ...
        for (Token<?> t : this.prevTokens) {
            ...
            if (t.getType() == Token.TokenType.Variable && t.getLexeme().equals(prevToken.getLexeme())) {
                isInit = false;
                break;
            }
        }
    ...
    private int getLookheadLexemeLength() {
        ...
        if (this.current.getType() == Token.TokenType.String) {
    ...
    private boolean factor0() {
        ...
        } else if (this.current.getType() == Token.TokenType.Number
                || this.current.getType() == Token.TokenType.String
                || this.current.getType() == Token.TokenType.Variable || this.current == Variable.TRUE
                || this.current == Variable.FALSE || isOPVariable(this.current)) {
            if (this.current.getType() == Token.TokenType.Variable) {
                checkVariableName(this.current);
            }
            // binary operation as variable for seq functions
            if (this.current.getType() == Token.TokenType.Char) {
    ...
    private FunctionParam lambdaArgument(final int index) {
        ...
        } else if (this.current.getType() == Token.TokenType.Variable) {
    ...
    private void pattern() {
        ...
        while (this.current != null) {
            ...
            if (getPrevToken().getType() == Token.TokenType.Char
    ...
    private void tryStatement() {
        ...
        while (this.current == Variable.CATCH) {
            ...
            move(true);
            if (this.current == null || this.current.getType() != Token.TokenType.Variable) {
    ...
    private void className() {
        if (this.current.getType() != Token.TokenType.Variable && !expectChar('*')) {
    ...
    private void forStatement() {
        ...
        boolean needCompatible = Token.TokenType.Char.equals(this.current.getType()) && "(".equals(this.current.getLexeme());
        if (needCompatible) {
            move(true);
    ...
    private DelegateToken anonymousMethodName() {
        return new DelegateToken(this.current, DelegateToken.DelegateTokenType.Method_Name);
    }

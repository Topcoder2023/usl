...
public class LoggerPluginEnhancer extends AbstractFunctionEnhancer {
    ...
    protected void enhancePluggable(FunctionPluggable fp) {
        ...
        if (fp instanceof Definable definable) {
            Class<?> targeted = definable.definition().getMethodMeta().targetType();
            skip = ArrayUtil.isNotEmpty(ReflectUtil.getFields(targeted, field -> Logger.class.equals(field.getType())));
        } else {
            skip = false;
        }
